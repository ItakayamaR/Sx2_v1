...
{ 
  ...
  status=receive_message(MODO, 20, true); //(Modulo de recepción, delay a esperar, con/sin mensaje de confirmación)
...
void Ini_module_spi(byte m) 
{
  ...
  LoRa.setSpreadingFactor(LORA_SF);             //Seteamos el Spreading Factor (SF)
  ...
  LoRa.enableCrc();                              //Activamos el CRC
